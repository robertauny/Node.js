...
function getCall(ip,ser) {
    ...
    const URL  = 'Some URL where the encrypted data files are kept.';
...
function reportNeededProducts(jovo,ep) {
    if( ep && ep.length > 0 ) {
        ...
        if( jovo ) {
            if( prds.length == 0 )
                jovo.tell('I could not find any requirements for your device.');
            else
                jovo.tell(`You currently require ${prds} .`);
        }
    }
    if( jovo ) jovo.tell('I could not find any requirements for your device.');
    return null;
}
...
app.setHandler({
    ...
    'WelcomeIntent': function() {
        this.ask('Welcome to the requirements prediction engine! What\'s your name?');
    },
    'MyNameIsIntent': function(name) {
        ...
        this.tell('Hey ' + name.value + ', nice to meet you! Let\'s see what are your current needs.');
        if( ip )
    ...
    'ShoppingIntent': function(serialNumber) {
        ...
        this.tell('Just a minute, ' + name.value + ', while I check your current requirements.');
